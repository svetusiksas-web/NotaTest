...
export const gradeFromPercentage=(percentage:number)=>Math.max(1,Math.min(10,Math.ceil(Math.max(0,percentage)/10)));
export function scoreTest(test:QuizTest,student:Student,answers:Record<string,Answer>):Attempt{const maxScore=test.questions.reduce((n,q)=>n+q.points,0);const score=test.questions.reduce((n,q)=>n+scoreQuestion(q,answers[q.id]),0);const percentage=maxScore?Math.round(score/maxScore*100):0;const grade=gradeFromPercentage(percentage);return{id:crypto.randomUUID(),testId:test.id,student,answers,score,maxScore,percentage,grade,completedAt:new Date().toISOString()}}
