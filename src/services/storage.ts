...
export async function initializeCloud(){try{const user=await waitForUser();const testsQuery=user.isAnonymous?query(collection(db,'tests'),where('status','==','published')):query(collection(db,'tests'),where('ownerId','==',user.uid));const cloudTests=(await getDocs(testsQuery)).docs.map(item=>item.data() as QuizTest);if(user.isAnonymous){if(cloudTests.length)write(TK,cloudTests);return}const localOrphans=testService.all().filter(test=>!test.ownerId);const migrated=await Promise.all(localOrphans.map(async test=>{const owned={...test,ownerId:user.uid};await setDoc(doc(db,'tests',owned.id),owned);return owned}));const tests=[...cloudTests,...migrated].filter((test,index,all)=>all.findIndex(item=>item.id===test.id)===index);if(tests.length)write(TK,tests);await refreshResults()}catch(error){console.warn('Firebase недоступен, используется локальное хранилище',error)}}
