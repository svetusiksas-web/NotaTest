...
export const resultService={all:()=>read<Attempt[]>(AK,[]),save:async(a:Attempt)=>{await waitForUser();const test=testService.get(a.testId);if(!test?.ownerId)throw new Error('У теста не указан преподаватель. Его нужно повторно опубликовать из кабинета.');const next={...a,ownerId:test.ownerId};await setDoc(doc(db,'attempts',a.id),next);write(AK,[next,...read<Attempt[]>(AK,[]).filter(item=>item.id!==next.id)]);return next}};
...
export async function initializeCloud(){try{const user=await waitForUser();const testsQuery=user.isAnonymous?query(collection(db,'tests'),where('status','==','published')):query(collection(db,'tests'),where('ownerId','==',user.uid));const cloudTests=(await getDocs(testsQuery)).docs.map(item=>item.data() as QuizTest);if(user.isAnonymous){if(cloudTests.length)write(TK,cloudTests);return}const localOrphans=testService.all().filter(test=>!test.ownerId);const migrated=await Promise.all(localOrphans.map(async test=>{const owned={...test,ownerId:user.uid};await setDoc(doc(db,'tests',owned.id),owned);return owned}));const tests=[...cloudTests,...migrated].filter((test,index,all)=>all.findIndex(item=>item.id===test.id)===index);const seen=new Set<string>();const uniqueTests=await Promise.all(tests.map(async test=>{const slug=!test.slug||seen.has(test.slug)?shortCode(test.id):test.slug;seen.add(slug);if(slug!==test.slug){const repaired={...test,slug};await setDoc(doc(db,'tests',repaired.id),repaired);return repaired}return test}));if(uniqueTests.length)write(TK,uniqueTests);await refreshResults()}catch(error){console.warn('Firebase недоступен, используется локальное хранилище',error)}}
